...
export class CacheManager {
  ...
  /** 页面快照缓存 */
  private pageSnapshots: Map<string, PageSnapshot>;
  ...
  getCachedExperiments(route?: string): Experiment[] | null {
    const key = route ? `experiments_${route}` : 'experiments';
    return this.get<Experiment[]>('experiments') || this.get<Experiment[]>(key);
  }
  ...
  getCachedExperimentResult(experimentId: string): ExperimentResult | null {
    return this.get<ExperimentResult>(`result_${experimentId}`);
  }
  ...
  getCachedUserBinding(userId: string): string | null {
    return this.get<string>(`binding_${userId}`);
  }
  ...
  getPageSnapshot(route: string): PageSnapshot | null {
    ...
    return this.get<PageSnapshot>(`snapshot_${route}`);
  }
